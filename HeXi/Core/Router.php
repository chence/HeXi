...
class Router {
    ...
    public function run($url, $withFilter = null) {
        ...
        if ($param[1]) {
            ...
            $controllerFile = config('controller.path') . $controllerName . '.php';
            #首先寻找和判断文件
            if (is_file($controllerFile)) {
                require_once $controllerFile;
                #然后是寻找控制器类
                if (!class_exists($controllerName, false)) {
                    error('控制器文件中没有定义控制器类 "' . $controllerName . '"');
                }
                #再后是生成控制器实例
        ...
        //-----------------------------
        #寻找默认控制器
        $controllerName = config('controller.default') . 'Controller';
        $controllerFile = config('controller.path') . $controllerName . '.php';
        if (is_file($controllerFile)) {
            require_once $controllerFile;
            if (!class_exists($controllerName, false)) {
    ...
    private function executeController(Controller $controller, $method, $filter) {
        ...
        if (!is_callable(array($controller, $method))) {
            if (!config('controller.method.capture')) {
                ...
                $method = config('controller.method.default');
            }
        }
        $this->param['controller'] = get_class($controller);
